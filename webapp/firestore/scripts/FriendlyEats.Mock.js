...
/**
 * Adds a set of mock tokens to the Cloud Firestore.
 */
FriendlyEats.prototype.addMocktokens = function() {
  var promises = [];
  ...
  for (var i = 0; i < 20; i++) {
    ...
    if (!promise) {
      alert('addtoken() is not implemented yet!');
...
/**
 * Adds a set of mock Ratings to the given token.
 */
FriendlyEats.prototype.addMockRatings = function(tokenID) {
  var ratingPromises = [];
  for (var r = 0; r < 5*Math.random(); r++) {
    ...
    ratingPromises.push(this.addRating(tokenID, rating));
  }
