...
FriendlyEats.prototype.viewList = function(filters, filter_description) {
  ...
  var renderResults = function(doc) {
    ...
    var data = doc.data();
    data['.id'] = doc.id;
    data['go_to_token'] = function() {
      that.router.navigate('/tokens/' + doc.id);
    };

    var el = that.renderTemplate('token-card', data);
  ...
  if (filters.city || filters.category || filters.price || filters.sort !== 'Rating' ) {
    ...
    this.getAlltokens(renderResults);
  }
...
FriendlyEats.prototype.viewSetup = function() {
  ...
  var notokensEl = this.renderTemplate('no-tokens', config);

  var button = notokensEl.querySelector('#add_mock_data');
  ...
  button.addEventListener('click', function(event) {
    ...
    that.addMocktokens().then(function() {
      that.rerender();
    });
  });

  this.replaceElement(document.querySelector('.header'), headerEl);
  this.replaceElement(document.querySelector('main'), notokensEl);
...
FriendlyEats.prototype.viewtoken = function(id) {
  var sectionHeaderEl;
  var that = this;
  ...
  return this.gettoken(id)
    .then(function(doc) {
      ...
      sectionHeaderEl = that.renderTemplate('token-header', data);
      sectionHeaderEl
