...
//const eth = new window.Eth(new Eth.HttpProvider('https://mainnet.infura.io'));
...
*/
    ...
    token.balanceOf(web3.eth.defaultAccount).then((balance) => {
    ...
    dropper.airdroppedUsers(web3.eth.defaultAccount).then((hasGottenAirdrop) => {
    ...
    $("#withdrawAirdropTokens").on("click", () => {
        ...
        dropper.withdrawAirdropTokens({"from": web3.eth.defaultAccount}).then((tx) => {
                $("#eligibility").html("Requested. Transaction ID: <a href='https://etherscan.io/tx/"+ JSON.stringify(tx)+"' />"+JSON.stringify(tx)+"</a>");
        })
    })
